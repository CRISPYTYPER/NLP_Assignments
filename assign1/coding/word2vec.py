...
def naiveSoftmaxLossAndGradient(
    ...
    p_o_c = softmax(outsideVectors @ centerWordVec)  # size (num words in vocab, ), a 1d vector
    loss = -np.log(p_o_c[outsideWordIdx])  # a scalar, which is a loss given u_o and v_c

    y = np.zeros(p_o_c.shape)  # a one-hot vector with a 1 at the outsideWordIdx
    y[outsideWordIdx] = 1

    gradCenterVec = outsideVectors.T @ (p_o_c - y)  # U(y_hat - y) in pdf, and U.T(y_hat - y) here
    # has size (word vector length, )

    gradOutsideVecs = np.outer((p_o_c - y), centerWordVec)  # size (num words in vocab, word vector length)

    # size check to verify the matrix operations
...
def negSamplingLossAndGradient(
    ...
    v_c = centerWordVec  # Center word vector (shape: (d,))
    u_o = outsideVectors[outsideWordIdx]  # Outside word vector (shape: (d,))
    u_ks = outsideVectors[negSampleWordIndices]  # Negative samples (shape: (K, d)

    p_o_c = sigmoid(u_o @ v_c)  # Scalar
    p_ks_c = sigmoid(-u_ks @ v_c)  # Shape: (K,)

    loss = -np.log(p_o_c) - np.sum(np.log(p_ks_c))

    gradCenterVec = -(1 - p_o_c) * u_o + np.sum((1 - p_ks_c[:, np.newaxis]) * u_ks, axis=0)  # size (d, )

    gradOutsideVecs = np.zeros(shape=outsideVectors.shape)  # size (V, d)
    gradOutsideVecs[outsideWordIdx] = -(1 - p_o_c) * v_c
    for i, neg_idx in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg_idx] += (1 - p_ks_c[i]) * v_c  # += because duplicate indices might appear
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    # Current center word is given as a parameter

    currentCenterIdx = word2Ind[currentCenterWord]
    currentCenterVector = centerWordVectors[currentCenterIdx]

    for outsideWord in outsideWords:
        outsideWordIndex = word2Ind[outsideWord]
        miniLoss, miniGradCenterVecs, miniGradOutsideVecs = word2vecLossAndGradient(currentCenterVector,
                                                                                    outsideWordIndex, outsideVectors,
                                                                                    dataset)
        # word2vecLossAndGradient is for one center word and one outside word,
        # thus need to accumulate for all the outside words

        loss += miniLoss
        gradCenterVecs[currentCenterIdx] += miniGradCenterVecs
        gradOutsideVectors += miniGradOutsideVecs
    ### END YOUR CODE
